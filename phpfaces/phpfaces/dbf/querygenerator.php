...
class QueryGenerator {
    ...
    var $select;
    private $super_class;
    function QueryGenerator(ClassMap $map,$sinif_adi="") {
        $this->class_map= $map;
    }
    function getQuery() {

        $table= $this->class_map->getTableName($this->super_class);
        $query = "SELECT $this->select FROM $table $this->birlestir";
        return $query;
    }
    function clear() {
    ...
    function doJoins($sinif_adi) {
        ...
        foreach ($sinif_haritasi as $tablo => $deger) {
            foreach ($deger as $oge) {
                ...
                if(!isset($this->islenmis_haritalar[$islenecek_sinif_adi])) {
                    $itable_name = $this->class_map->getTableName($islenecek_sinif_adi);
                    // $itable_name=$islenecek_sinif_adi;
                    $harici_anahtar = $oge["fk"];
                    $dahili_anahtar = $oge["pk"];
                    $join = " LEFT JOIN $itable_name
                    ON $itable_name.$harici_anahtar
                    =  $table_name.$dahili_anahtar";
                    $this->birlestir.=$join ;
    ...
    function doColumns() {
        $list = $this->class_map->getClassList();
        $i=0;
        $this->select="";
        foreach ($list as $class_name) {
            $sinif_haritasi=$this->class_map->getClassColumns($class_name);
            $table_name = $this->class_map->getTableName($class_name);
            foreach ($sinif_haritasi as $column) {
...
class Record {
    ...
    var $db;
    var $changeSupport;
    function Record(ObjectRegistry $registry,ClassMap $map,$db) {
    ...
    public function save($entity) {
        ...
        if($this->registry->isRegisted($entity)) {
